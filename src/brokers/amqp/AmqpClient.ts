...
export class AmqpClient {
	...
	private rpcCallbacks: {
		[correlationid: string]: (err?: any, payload?: any) => any;
	} = {};

	private async getPayload(r: Message): Promise<any> {
	 ...
	 */
	public get baseRpcQueue(): string {
	...
	public constructor(
		private existingHooks: IAmqpConnectionHooks,
		private clientOptions: Partial<AmqpClientOptions>
	) {}

	public async init(): Promise<void> {
		if (this.clientOptions.newChannel) {
			this.channel = await this.existingHooks
				.getConnection()
				.createChannel();
		} else {
	 ...
	 * @param msg
	 */
	private consumeRpcMessage(msg: ConsumeMessage | null): void {
		if (!msg) {
			return;
		}
