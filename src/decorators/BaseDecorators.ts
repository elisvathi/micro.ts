 ...
 */
export function registerHandlerMetadata(
	target: any,
	propertyKey: string,
	...
	let controller: { [key: string]: MethodDescription } = metadata.methods.get(
		target
	) as { [key: string]: MethodDescription };
	...
	controller[propertyKey].metadata = {
		...(controller[propertyKey].metadata || {}),
		...(options || {}),
	};
	...
	if (existingParams && existingParams[propertyKey]) {
		controller[propertyKey].params = existingParams[
			propertyKey
		] as ParamDescription[];
	} else {
		...
		if (paramtypes && paramtypes.length) {
			controller[propertyKey].params.push(
				...paramtypes.map((x) => {
					return { type: x };
				})
			);
 ...
 */
export function registerParamMetadata(
	target: any,
	propertyKey: string,
	index: number,
	...
	let controller: { [key: string]: ParamDescription[] } =
		metadata.parameters.get(target) as {
			[key: string]: ParamDescription[];
		};
	controller = controller || {};
 ...
 */
export function attachHandlerAuthorization(
	target: any,
	propertyKey: string,
	_descriptor: PropertyDescriptor,
 ...
 * @param options
 */
export function attachHandlerMiddleware(
	target: any,
	propertyKey: string,
	_descriptor: PropertyDescriptor,
...
export function attachHandlerRedirect(
	target: any,
	propertyKey: string,
	_descriptor: PropertyDescriptor,
	url: string
...
export function attachHandlerBrokersFitler(
	target: any,
	propertyKey: string,
	_descriptor: PropertyDescriptor,
	options: BrokerFilter
): void {
...
export function attachHandlerErrorHandler(
	target: any,
	propertyKey: string,
	_descriptor: PropertyDescriptor,
	options: AppErrorHandler[]
): void {
	const metadata = getGlobalMetadata();
	let controller: { [key: string]: MethodDescription } = metadata.methods.get(
		target
...
export function registerControllerMetadata(
	target: any,
	options?: ControllerOptions
): void {
	const metadata = getGlobalMetadata();
	metadata.controllers =
		metadata.controllers || new Map<any, ControllerMetadata>();
	const name: string = target.name;
	// let found = metadata.controllers.find(x => x.ctor === target);
	let found: ControllerMetadata | undefined =
		metadata.controllers.get(target);
	...
	if (options && options.errorHandlers) {
		const currentErrorHandlers: AppErrorHandler[] =
			found.options.errorHandlers || [];
	...
	if (options && options.middlewares) {
		const currentMiddlewares: MiddlewareOptions[] =
			found.options.middlewares || [];
...
export function attachControllerErrorHandlers(
	target: any,
...
export function attachControllerMiddleware(
	target: any,
...
export function attachControllerAuthorization(
	target: any,
	options?: AuthorizeOptions
): void {
	registerControllerMetadata(target, {
