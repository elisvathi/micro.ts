...
function buildControllerSchema(metadata: ControllerMetadata) {
	...
	if (metadata.handlers) {
		Object.keys(metadata.handlers).forEach((key) => {
			const obj = buildHandlerSchema(
				'',
				metadata.options?.path || '',
				metadata.ctor,
				metadata.handlers![key]
			);
			methodSchemas[obj.path] = methodSchemas[obj.path] || {};
			const newObj: Partial<typeof obj> = { ...obj };
			delete newObj.path;
...
function buildHandlerSchema(
	...
	return {
		...
		[description.metadata?.method || 'get']: buildMethodParameters(
			description,
			ctor.name
		),
	};
...
function buildMethodParameters(method: MethodDescription, controller: string) {
	const obj: OpenAPIV3.OperationObject = {
	...
	method.params.forEach((param) => {
		if (param.options?.decoratorType) {
			...
			if (param.options.decoratorType === ParamDecoratorType.ParamField) {
				...
				if (
					!obj.parameters.find((x: any) => x.name === name && x.id === 'path')
				) {
					obj.parameters.push({
						name,
						in: 'path',
			...
			if (param.options.decoratorType === ParamDecoratorType.QueryField) {
				...
				const name = param.options.name;
				if (
					!obj.parameters.find((x: any) => x.name === name && x.in === 'query')
				) {
					obj.parameters.push({
						name,
						in: 'query',
			...
			if (param.options.decoratorType === ParamDecoratorType.HeaderField) {
				...
				const name = param.options.name;
				if (
					!obj.parameters.find((x: any) => x.name === name && x.in === 'header')
				) {
					obj.parameters.push({
						name,
						in: 'header',
